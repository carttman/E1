...
public class BezierProjectile : Projectile
{
    ...
    private readonly float maxAllowedDisplacement = 3f;
    ...
    protected override void OnEnable()
    {
        ...
        middlePos = Vector3.Lerp(startPos, target.position, 0.5f) +
                    (target.position - startPos).magnitude * upwardMovementModifier * Vector3.up;
    ...
    protected override void OnUpdate()
    {
        ...
        if (displacement > maxAllowedDisplacement) Release();
        ...
        if (age >= duration)
        {
            ...
            if (target && target.GetComponent<StatsComponent>() is StatsComponent sc) sc.TakeDamage(DamagePacket);
