...
public class TowerBuildButtonUI : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    private void OnGoldChanged(int gold)
    {
        ...
        {
            _goldChangeTween?.Kill();
            _goldChangeTween = _rectTransform.DOLocalMoveY(_startYPosition, 0.2f).SetEase(Ease.OutElastic)
                .SetLink(gameObject);
        }
        else
        {
            _tween?.Kill();
            _goldChangeTween?.Kill();
            _goldChangeTween = _rectTransform.DOLocalMoveY(_startYPosition - pointerTweenOffset, 0.2f)
